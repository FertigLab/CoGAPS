...
\par Before installing the CoGAPS R package, standard R procedures should be followed to install the package \texttt{R.utils}.  If \$GAPSJAGS\_PATH the default \texttt{/usr/local} directory, CoGAPS can be installed with the standard Bioconductor installation commands.  Otherwise, the following subsections contain instructions to install the CoGAPS package on the Unix / Mac and the Windows operating systems.
