...
\par Before installing the CoGAPS R package, standard R procedures should be followed to install the package \texttt{R.utils}.  Once installed, the following subsections contain instructions to install the CoGAPS package on the Unix / Mac and the Windows operating systems.
...
\begin{verbatim}
source("http://prs.ism.ac.jp/biocLite.R")
bioCLite("CoGAPS", configure.args="--with-jags-include=${GAPSJAGS_PATH}/include/GAPS-JAGS  --with-jags-lib=${GAPSJAGS_PATH}/lib --with-jags-modules=${GAPSJAGS_PATH}/lib/JAGS/modules-1.0.2")
\end{verbatim}
...
\par Alternatively, the CoGAPS package can be obtained through \url{http://www.cancerbiostats.onc.jhmi.edu/CoGAPS.cfm}.  In this case, download the CoGAPS source and install using the following command line argument:
\begin{verbatim}
