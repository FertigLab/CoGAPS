...
setMethod('setParam', signature(params='CoGapsParams'), 
    function(params, whichParam, value)
    {
        ...
        if (whichParam == "nIter")
            params@nIter <- value
        if (whichParam == "outputFrequency")
            params@outputFrequency <- value
        if (whichParam == "alphaA")
            params@alphaA <- value
        if (whichParam == "alphaP")
            params@alphaP <- value
        if (whichParam == "maxGibbmassA")
            params@maxGibbmassA <- value
        if (whichParam == "maxGibbmassP")
            params@maxGibbmassP <- value
        if (whichParam == "messages")
            params@messages <- value
        if (whichParam == "singleCellRNASeq")
            params@singleCellRNASeq <- value
        if (whichParam == "whichMatrixFixed")
            params@whichMatrixFixed <- value
        if (whichParam == "fixedMatrix")
            params@fixedMatrix <- value
        if (whichParam == "checkpointInterval")
            params@checkpointInterval <- value
        if (whichParam == "checkpointFile")
            params@checkpointFile <- value
        if (whichParam == "nCores")
            params@nCores <- value
        # DONE(Hyejune) repeat this for all parameters
