...
modsimresult <- readRDS("../data/ModSimResult.Rds")
```
...
```{r load single cell data from file, eval=FALSE}
...
pdac_data <- readRDS("../data/inputdata.Rds") # load R data object 
pdac_data
```
...
-   A higher score value indicates the marker gene is more associated
    with the pattern, and vice versa.l
...
```{r load original data object again, eval=FALSE}
...
```{r MANOVA variables, eval=FALSE}
# interestedVariables <- cbind(colData(monocleobj)[["TN_assigned_cell_type"]], colData(monocleobj)[["Classifier_T_duct"]])
# 
# manovaResult <- manova(interestedVariables, cogapsresult@sampleFactors)
```
