...
author: "Thomas Sherman, Genevieve Stein-O'Brien, Jeanette Johnson, Ashley Tsang, Hyejune Limb, Elana Fertig"
...
Coordinated Gene Association in Pattern Sets (CoGAPS) is a technique for latent space learning in bulk gene expression or single cell RNA sequencing data. CoGAPS is a member of the Nonnegative Matrix Factorization (NMF) class of algorithms, which factorize a data matrix into two related matrices: the Amplitude (A) matrix which contains gene weights, and Pattern (P) Matrix which contains sample weights. Each column of A or row of P defines a feature and together this set of features defines the latent space among genes and samples, respectively. In NMF, the values of the elements in the A and P matrices are constrained to be greater than or equal to zero. This constraint simultaneously reflects the non-negative nature of gene expression data and enforces the additive nature of the resulting feature dimensions, generating solutions that are biologically intuitive to interpret (@SEUNG_1999).

This package presents a R interface for CoGAPS along with the underlying implementation in C++. A python package for CoGAPS, PyCoGAPS, is now available here: https://github.com/FertigLab/pycogaps

# Installing CoGAPS

*CoGAPS* is a bioconductor package. The current, stable release can be installed as follows:
