...
## Setting Parameters
Although default parameters are set, the user may change and set the parameters. 
For example, if we wanted to change the number of patterns from the default to 5,
```{r}
params <- new(CogapsParams)
params <- setParam(params, "nPatterns", 5)
cogapsResult <- CoGAPS(GIST.D, params)
...
# Session Information and Package Compilation
The following will detail the session information and information about how the package was compiled, the version used, and compile time options.
```{r}
sessionInfo()
buildReport()
```
