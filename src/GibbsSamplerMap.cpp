		...
		case 'A':{
			...
			for (int iCol = 0; iCol < _nFixedMaps; iCol++){
				...
				for (int iAtom = 0; iAtom < _nRow; iAtom++){
					updatebin = getAtomBin('A', iAtom, iCol);
		...
		case 'P':{
			...
			for (int iRow = 0; iRow < _nFixedMaps; iRow++){
				...
				for (int iAtom = 0; iAtom < _nCol; iAtom++){
					updatebin = getAtomBin('P', _nFactor - _nFixedMaps + iRow, iAtom);
					
					// EJF note: previously used this code, but introduced a VERY strange
					// wrap around error that could not be resolved
					// must be debugged!!!!!
					// updateloc = _PAtomicdomain.getMidLocation(updatebin);

					
					// Place the atom at the start of the bin
					updateloc = _PAtomicdomain.getStartLocation(updatebin)+1;
